...
5. **Clamp GI ray intensity** — `r.LumenScene.Radiosity.MaxRayIntensity` (he cites the default as ~40) clamps the contribution of individual bright rays. He first drops it to `1.0` to show the effect at an extreme [frame_003], then settles on `4.0` [frame_001], trading a little GI brightness for markedly less fireflying and flicker [transcript 4:27–5:03].
6. **GI temporal window** — `r.LumenScene.Radiosity.Temporal.MaxFramesAccumulated`, raised through 64 → 1024 → 4096. This is the single biggest lever on flicker, at the cost of slower response to changing light [frame_001 (64), frame_007 (1024), frame_009 (4096)].
   > **The engine default is 4, not "4 or 8".** Hovering a row pops a tooltip reading `Custom Value: 4 / Preset Value: 64 / Startup Value: 4 (Set By Constructor)` [frame_004]. That tooltip is the reliable way to read a cvar's true default in this panel — the narration guesses at defaults more than once and is not a safe source for them.
7. **Reflection temporal window** — the same idea on the reflection path: `r.Lumen.Reflections.Temporal.MaxFramesAccumulated` at `1.0` leaves the close-up reflective surface visibly blotchy and noisy [frame_005], while 1024/4096 is dramatically softer and more stable [frame_007, frame_009, transcript 6:56–7:13]. `r.Lumen.Reflections.MaxRayIntensity` clamps reflection rays the same way (set to 4).
8. **Surface cache resolution** — `r.LumenScene.SurfaceCache.CardMaxResolution` driven down to `1` to show the loss [frame_006], then up through 1024 to 4096; low values lose detail in the cached surface representation that both GI and reflections read from [frame_007, frame_009]. Note he has already set `VisualizeProbes 0` here — the surface cache is judged on the beauty image, not the probe view [frame_006].
...
10. **Two optional look controls, demonstrated but not kept** — `r.Lumen.Reflections.SpecularScale` pushes specular beyond physically-based values toward a path-traced feel (he warns it blows out quickly and is not PBR-correct); `r.Lumen.Reflections.Contrast` slightly reduces overall reflection strength, also not physically correct [transcript 9:31–10:26].
    > **Neither survives into the final state.** `SpecularScale` still reads `1.0` with `Source = Constructor` — i.e. never changed this session — in both [frame_008] and [frame_009], and `Contrast` only ever appears in the **+ Console Variable** autocomplete [frame_009], never as a row. Treat both as look options he showed and backed out of, not part of the recipe.
...
| 9 | `r.Lumen.Reflections.SpecularScale` | Specular multiplier; >1 approaches a path-traced look but breaks PBR. **Demonstrated only — stays `Constructor`/unchanged in every frame** | `1.0` (untouched) |
...
Also demonstrated but never added as a row: `r.Lumen.Reflections.Contrast`, seen only in the **+ Console Variable** autocomplete, which is what confirms its spelling [frame_009].

**Post Process Volume context** [frame_000] — the scene's `Hallway_Main_PostProcessVolume` runs `Reflections → Method = Lumen` with `Quality 1.0`, `Screen Traces` on, `Max Roughness To Trace 0.4`, `Max Reflection Bounces 1`, and **MegaLights enabled** under `Rendering Features`. This is the layer the whole tutorial deliberately works *around*.

**Reading true defaults:** hovering a row in the Console Variables Editor shows `Custom Value` / `Preset Value` / `Startup Value (Set By Constructor)` [frame_004] — use it rather than trusting a remembered or narrated default.
...
### UE Version
Not stated in the video. Two independent UI signals place it at **UE 5.5 or later**: the Content Browser shows the integrated **Fab** button (Fab replaced Quixel Bridge in 5.5) alongside Zen Server and Revision Control status widgets [frame_001], and the Post Process Volume exposes **`Rendering Features → MegaLights`**, introduced as Experimental in 5.5 [frame_000]. Treat 5.5+ as an inferred lower bound, not a stated version.
