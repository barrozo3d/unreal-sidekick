...
**Transcript:** mesh that we have generated because what we got from the 3D generator from EVO  3D is not a great topology and I'll show you that once we pull it into here.  And in order for it to work with the metahuman creator, it needs to match the metahuman topology.  And so you need a piece of software like this that can take the metahuman topology and  rap it around this.  And so that's how I know how to use this.  And I will show you how to do that right now.  So over here in the graph, we are going to create some load geometry nodes.  So this first one we will use to load the metahuman topology.  So I'll click on that.  And then here you'll see there's a metahuman body topology and a metahuman head topology  that will be included in the project files linked below in the description.  So let's grab the head and open that up.  And when that comes in, let's zoom out here.  It's going to look like that.  So come down here and let's set the rotation to minus 90.  And it's going to be up there and down all to move.  Gosh, every 3D software is like different controls to move around.  Anyway, all right.  So that's our metahuman topology.  Let's go ahead and create another load geometry node.  And this is the one that we will use to load our demon guy.  So we'll go ahead and select that one.  And we just go ahead and load single file.  OK.  And as you can see, that loaded in down there.  So let's just grab that and we'll scoot it up.  And it's a little bit small.  So let's try scaling that up times two.  It's a little big.  So let's just try to get it close to the other 1.8.  Let's set it.  Yeah.  Doesn't have to be perfect.  I think that looks good.  Then we're going to right click.  And we're going to create a select point pairs node.  And we are going to hook our two geometries up to that.  So grab those little pickwits there and connect them to there.  And now if you actually select that node  and you come over here to this tab over here,  you can come to the visual editor.  And you'll see that we have them both side by side.  And if you come down here and then click sync views,  and then you'll now be able to move around.  And it will sync with it.  Gosh, I can never remember which ones to click.  If you hit F, that will actually focus it.  And then hold alt and left click.  And now you can actually move stuff around properly.  OK.  You can also hit S. And it will snap to the nearest viewpoint.  So that is a little helpful.  OK.  So now we want to get some actual materials on our meshes.  And so let's come up here and we'll  go and add a load image node.  And in here, we will select the material for our metahuman head  that comes with the files for this project.  Go ahead and select that.  And you can just then connect it to there.  And then you can just connect the load image to there.  And if we come down here, that is starting to show up.  And then over here, we'll add another load image.  And we'll load the color.  We'll load the diffuse map of the demon.  So select that and connect those.  And if we come over here, great.  By the way, if you come up here and select the geometry,  you can turn off things like the wireframe.  So I'll just do that really quickly.  And then come back here and select that head F.  So that's going to make it a little bit easier  as we apply the points.  So we're going to add some points on both of these that  will help it figure out exactly which part of this mesh  should go onto this mesh.  So we'll come over here and click on this little node here  and pull that off and then right click.  And we are going to add a cartoon wrapping node.  And this is the node that does all the magic.  And there's a regular wrapping node.  But I found that using the cartoon one  works best when actually wrapping something a little bit  slightly less humanoid.  In fact, it might also have to do with the mesh topology  of the 3D mesh generator.  It might just need the more extreme cartoon wrapping node  to work with this.  So anyway, just use that.  I know it works.  And what we want to do is we want  to connect the geometry nodes to these points  on the cartoon wrapping node.  Great.  All right.  So now we can click on the cartoon wrapping node.  And it's going to start to look like this.  And we can see the metahuman geometry  overlaid on our demon.  OK.  All right.  So let's come back to the select point pairs node over here.  And make sure you're in the Visual Editor tab.
...
**Transcript:** And we're going to start adding points to these.  OK.  So what does that mean?  I'm going to start clicking over here  and assigning points on this geometry  and then lining up corresponding ones on this geometry.  And that's basically to be like, oh, the corner of the eye  on this mesh goes in here.  And like the corner of the mouth goes here and so on.  And we'll do some parts around the horns  to make sure we get some of this geometry to wrap around those.  And the more points you can put the better,  I've been doing between 50 and 100 points  based on how much extra geometry there is.  So we'll just get started.  And we'll see how far we get with this one.  I don't think it should be that many, but we'll see.  So the first thing I'm going to do  is come over to this left geometry tab, which corresponds  to this area over here.  And where it says symmetry, I'm going to turn on x symmetry.  So now you can see as I'm moving my cursor,  there's a corresponding point over there.  So let's just start with these.  And you see there's a one and a two there.  And then you'll see up here, point count is two.  And then over here, point count is zero, right?  OK.  So we want these to stay the same.  So if you start clicking along, and you get to 36 over here  and 32 over here, you messed up.  You want them to be the same.  OK.  So over here, we're going to leave this on.  I'm going to click on the right geometry tab.  And we're going to leave it at no symmetry.  So when we're over on this viewport,  we place the points individually.  So we got one, two, we'll go one, and two.  And we can kind of work around.  Yeah, that looks great.  Yep.  And that's just because the symmetry  doesn't seem to work as well on this mesh, the 3D generated one.  OK.  So now we're just going to start placing points all over.  And like I said, the more the better.  But I'm going to speed this up quite a bit here.  But you can see how I'm just kind of going into the really  detailed areas and trying to just give as much information  to wrap as possible.  Really going over the main features, the corners of the mouths  and you can see I'm clicking along both the top and the bottom  of the lip to kind of show where the mouth is parted along  the jawline, the cheekbones, dimples along the cheeks,  creases on the neck.  And so you can see that I'm also keeping track of the numbers  as I am clicking.  And like I said, if you ever start to get out of sync  with those numbers, it's best to just back up  to where they're in sync and then keep going.  And actually that happens to me in this part right here  as I am clicking around the horns.  You can see how I'm kind of like putting, yep,  and there I'm undoing all of those and just backing up  to where the points are back in sync.  But you can see how I'm kind of clicking around to show  where the wrap should go on the horns.  And then around the back of the head,  just to get a little bit more information back there.  And there we go.  And now we can come over to the cartoon wrapping node  and click on that.  And we should be able to just hit compute
...
**Transcript:** and it should do the wrap for us.  And now this is sped up about 10X,  but you can see the metahuman topology  start to form over the AI generated topology.  Okay, that's looking pretty good,  but what we wanna do is we wanna actually add another node  over here, wanna add a brush node.  And this will allow us to sort of clean up the wrap.  And the way we wanna set this up is to take the cartoon wrapping  and plug that into the far left node here.  And then we're gonna take this load geometry one.  We're gonna bring it down to this third one here.  And then we're gonna take the other load geometry.  And we're gonna bring that down.  We're gonna put it in the second one there.  Okay, and so it's gonna start to look like this.  So what we wanna do with the brush node selected  is we wanna kinda clean up the geometry here.  And so we've got the wireframe turned on.  And if you don't see the wireframe,  you can scroll over here and click on that.  You can also hide it.  And right now it says display mode texture.  Let's switch this to color.  I think it makes it a little bit easier to see.  And so you can see kinda like these areas  where the meshes creased over all this wrap  did really, really well actually.  It compared to some of the other ones.  You know, there's really just some stuff in the kind  of the eyelids, like some weird, you know,  fold there or something, maybe up around these horns.  You see there's kinda some geometry here.  And we wanna just clean that up.  And so right now I have the relaxed tool selected.  And so, oops.  The tool was way too strong.  So let's come over here and just turn that down a lot.  And okay, well, you know,  that's kind of cleaning that up a little bit more  than I would like.  Let's turn the strength down even more.  Maybe we'll turn the radius down.  Okay, that looks good enough.  And here I'm gonna do the other horn.  And then, you know, I turn the strength back up  and then, you know, I'm just cleaning up the under the eyelids.  You know, wherever that meshes really overlapping  or, you know, it looks like some vertices  are kinda like squished together.  But overall, this was a really good wrap  and there wasn't too much to clean up.  Okay, and as soon as you're done cleaning up the mesh,  you can come over to your brush node and hit accept.  And that will lock in those changes.  And then you come over here and you can right click  and come down and go to save output jom.  And we'll save the OBJ file  and we'll call this demon.
...
**Transcript:** And we can also change the body over in this tab.  And if you click on the body tab and then come over to model,  you know, you have all of these things that you can change,  you know, you can really sculpt your own, you know,  character as much as you want.  But I'm actually interested in a tab here that should be here.  And that is the, you know, original metahuman bodies.  And you can actually enable that if you don't see that here  in your project settings, you can go up to project settings  and you can scroll down to the plugins over here  and click on the metahuman character  and go to show compatibility mode bodies  and go ahead and close that.  And I think you just have to close the character creator  and just open it back up.  And now if we go to body and model and great,  we have the fixed compatibility  and you can see all the original metahuman bodies there.  So I am just gonna put some armor on this guy  because I don't have time to do the whole body  for this tutorial that will be in a later video.  But let's just do the normal male tall body type.  And that's just because I have like some armor  that can fit this guy.  Okay.  And if you click on the materials tab over here,  we can now apply the texture to the head.  So come down here to enable texture overrides  and click that and for the face, we can come down here  and we will do base color  and we can just search demon wrapped D.  Great.  All right, that's starting to look pretty cool.  Keep in mind a lot of this detail is baked  like this lighting detail,  but still looks pretty good I think  and it'll look even better.  We can come down here and I think we can add more,  we can add the normal map in here.  So we'll just go down here and search demon wrapped N.  And there we go.  That's already looking better.  Okay, so I'm not gonna go too much into  all the different sculpting you can do with the head.  Obviously like I mentioned, you can add transforms,  you can go to sculpt and really move stuff around,  but I really like how this turned out.  So I'm not gonna mess with it too much.  I'm actually just gonna go over here  and we're gonna go to create full rig.  And once that's done, let's go over to download textures  and we will download 8K.  And once that's done, you can come over here to assembly  and come down here and just click assemble.  All right, and now that that's assembled,  we can come up here and we can save and close that  and come down here and save all.  Gotta watch out for crashing magoo.  And we can come down to Metahumans
