---
title: Faster than AI and 7 times the fun! Speed up Animation and get exactly what you want with Rokoko
source: YouTube
url: https://www.youtube.com/watch?v=o_AE2kUsF3E
author: Dean Yurke - Unreal Engine and VFX Filmmaking
ingested: 2026-06-23
ue_version: "UE5"
tags: [mocap, rokoko, animation, retargeting, fbx, sequencer, composure, smartsuit, hand-tracking, beginner]
extraction_status: complete
frames_dir: tutorials/frames/faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w/
frame_count: 16
---

# Faster than AI and 7 times the fun! Speed up Animation and get exactly what you want with Rokoko

**Source:** [YouTube](https://www.youtube.com/watch?v=o_AE2kUsF3E)
**Author:** Dean Yurke - Unreal Engine and VFX Filmmaking
**Duration:** 14m44s | 16 section(s)

---

## Raw Data (for Claude Code extraction)


### Intro & The "Robot" Unboxing [0:00]
**Transcript:** So I'm very excited because the Coco have crazily given me one of their suits to play with and giving my thoughts on  My first thought though is where do I begin because some of your boxes and cables and Wi-Fi routers  It's  So I'm gonna let this guy over here  I'll come back when it's all done  The Coco is easy to set up. Let's do this thing make a mocap fun  So everything remarkably set up very quickly. I just follow the tutorials on the website and the website and  Plugged in the suit and the gloves and got the latest firmwares and  It all just did its magic and then worked and my biggest fear was because I'm down the end of my garden  And I've got a little room that I use to do all my videos and I have like a 300 foot cable  Ethernet cable and so I don't have like a Wi-Fi router in here  And I was a bit worried about having to set one up because it just scares me  But I used the mobile hotspot on the actual PC and it worked so  Brilliant, yeah, it's great and so  This was like two hours after opening the box and so I'm just gonna show you me  plotting through to create that opening scene and I'm gonna have to say that  The Ricoco provided the suit and they just came out of the blue and went and would you like to play with a suit?  And I was like, yes, please. It's great. And so I have to declare that so  Take it with that in mind, but I love it anyway, but I'll skip to the end. We didn't know it's great. It's really fun  Okay, so here we go. Here's the video  Create new scene. I'm going to call this one robot

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_000.jpg

### Setting Up the Scene in Rokoko Studio [1:52]
**Transcript:** robots, okay  Seems to be seeing me. That's a good sign. Now I think you have to go into a selection actor  And I already made an actor. So there's me and then you drag this onto the actor. So there we are  So that's my character now and now I have to go and do a  Quick pose. I've also got the coil so I'm just gonna do this one first  I'm just gonna try it out on its own. So I go is it that one the calibrate pose?

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_001.jpg

### Calibrating the Smartsuit Pro II [2:15]
**Transcript:** Six seconds. I made a six second because it takes me a little while to walk over here  Okay, look at that. That's brilliant. That's amazing. Now with just the suit on this is the suit  no coil and

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_002.jpg

### Testing Movement: Waving, Crouching, and Sitting [2:37]
**Transcript:** I mean that's pretty amazing  That's pretty good. So you can wait around and  You can crouch down. That's always a pain to do in animation people sitting down  So there you can see me on my screen over there and then I'm doing off-line stuff  So I'm doing animation. I'm gonna record it on  There's Julio and then I'm going to bake it down because there's some tools in there to clean up the mocap  But if you want to do live stuff and if you're a live streamer then  I mean it's fantastic, isn't it? I mean I'm  I'd be happy with just this but then there's more  So I'm gonna put the gloves on now. So these are the smart gloves  They've just come out with a smart gloves too, but these are the

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_003.jpg

### Adding the Smartgloves [3:18]
**Transcript:** Current smart gloves for you. Oh look at this. It's sort of seen one of my gloves there already  So that's great. I think I put the other glove in there's two gloves. So you got two gloves  And then do I drag this on like that?  Look at that. You just drag them on and now I've got the the fingers working now  This is the thing without the coil  The the gloves work, but they don't really have a sense a real good sense of 3D space  And so now I'm going to put the coil on  But I mean without the coil this is pretty great and this is live rather than in a minute once it once when I record this stuff  I'm going to be  Locking the feet are using their software, but because I I don't really need live broadcast  But  So that it's pretty good pretty nuts. Alrighty. Okay, now saying now I'm going to put this on this is the coil  And this is their EMF  Generator which sounds very

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_004.jpg

### Using the Coil Pro (EMF Generator) for Precision [4:20]
**Transcript:** sci-fi. So I like it  So let's put this on  I'll be more in here  There we are. Alright, so now this is booting up and we'll see what happens. Oh there you go  So it's seen the coil now  I've got my hands together. I can't see because my eyes aren't like it  So I'm trying very good works. I look like worked  So after I got it all set up and work in great  Next was to actually make the little intro. I'd already filmed a bit with me opening the boxes  And I was going to do the elaborate unboxing but I thought as boring  So let's just get the robot to do it much better

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_005.jpg

### The Immediacy of Mocap vs. AI Processing [5:00]
**Transcript:** So I had that and what I wanted to do was just play that on a loop  So then I could kind of have that and act against it as the robots  The thing that I really like about the rococo system is that it's live and there's no waiting  So you can perform you can see it take you can see that it's not very good and then you do it again  I tried some sort of the some of the AI kind of the ones where you upload it to a site and  They're good  But they're kind of annoying because you have to wait so long you have to film it you have to make sure you fit within the frame  And then also if anything's included it doesn't understand what that is  And then the worst thing is though it's just waiting and as you can tell I'm always in a rush  I've got time to waste um  So I like the immediacy of just seeing it's like working with an actor  You know you see if it takes good and then you go what do you think like that? I hope they'll change it  So there we go anyway, so I said on a loop and then I went and filmed myself  Embarrassing me in my pajamas. It's a rococo suit. It just looks like pajamas  She gets super on one all right on the show  Yeah  All right, that's my recording. Let's see how that worked  So I've unplugged the suit and

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_006.jpg

### Recording the Performance [6:21]
**Transcript:** Take a my 1980s headband off oh  Yeah, so I'm in rococo studio and this is my recording and  um  Along the bottom here you've got your foot locks and if I turn this on it basically calculates where it thinks your feet

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_007.jpg

### Post-Processing: Foot Locking and Drift Cleanup [6:37]
**Transcript:** Are planted on the ground and so you can edit this because with an inertia-based system even with coil  You still get drift and it's not  Outside in tracking  So it's um this helps fix all that so I'm going to go through and I can change these so wherever I think the foot should be locked  I can edit these and keep them locked like here  It's like there it needs to be locked there it needs to be locked so though  Anyway, so I'm just going to go through and manually do this and this is the  Second time I've opened up this software so for ease of use it's pretty damn  Use a friendly so far  This is great though to have this um cleanup. I mean it used to be so difficult  All right, there we are that's it. Okay now. I think I hit finalize post process  So now it's going to process that and clean it up a bit  All right, there we go now um, I'm going to go and export that to unreal engine  So one thing that I wanted to mention was that when I was first setting up the coil pro  I noticed that it was quite sensitive around anything magnetic  So if I was walking by a cable I the little suit would jump or it would kind of jitter or there was like weird thing  So it needed to be a little bit more. I haven't quite worked out yet  But the important thing was even though it did these little weird things when I recorded the animation  It didn't record that so I don't know if it's just a display thing or if it was just part of the cleanup software

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_008.jpg

### Sensitivity Tips for the Coil Pro [8:13]
**Transcript:** But if you're doing offline stuff then it's not really a problem if you're doing live  Streaming type stuff then it probably is a problem um talking of cool cleanup things um, I didn't know this  But after I processed all my animation and sent it out as an fbx great  That's what I used in the final video  But I also found that there's another piece of software that Rikoko have created called Rikoko studio preview  And it basically it's like a cleanup thing  So if you got jitter or noise in your motion capture which I didn't it'll smooth that out  And then also it lets you create loops out of animation as well  But I don't know why it's not in the main software because I didn't know about it

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_009.jpg

### Exploring Rokoko Studio Preview for Extra Cleanup [8:53]
**Transcript:** And I didn't see it on any of the onboarding videos um, so I guess it must be pretty new  But um, it's really cool so it's nice to have other things  To improve something that's already really good. So hooray. All right, so now let's bring that fbx animation into unreal engine  So I'm inside unreal engine and I've got my little robot character here  So I'm going to bring in now the animation  That we just recorded with the mocap and then I'm going to go right mouse button  Import to current folder and then I'm going to find my clip  Double click on that and I'm just going to say yep import and keep it all as default  There we go create physics asset skeleton and then I'll bring in the anim

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_010.jpg

### Importing Mocap Data (FBX) into Unreal Engine [9:41]
**Transcript:** And if I just double click on my anim file  Here it is. So there's my motion capture  And he's doing his magic stuff  Not the most like dynamic and exciting of motion captures is it  um  But who cares because he's telling my story and it's saving me hours and hours and hours and hours of animating  In fact, I wouldn't have bothered animating it  I would have just had a locked off  There we are. So we've increased our entertainment  Factor by seven  Yes  All right, so I'm going to retarget this anim onto this character right mouse button  Retarget animations  And then we've got our source skeletal mesh and then we're going to choose our target mesh  Which is a robot pilot  There he is and now that's the animation that it came with this character. So if I double click on that  It will now retarget onto this guy  Oh  Fun  There we are  Beautiful  Beautiful stuff and then I'm going to hit export animations

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_011.jpg

### Retargeting Animation to a Robot Character [10:45]
**Transcript:** And I'm going to give it a name a prefix where I'm going to put it  I'm going to put it into my models characters central robot converted  There we are and hit export  And then hit export  Bing bong bong  There we are and now what I can do just to test it if I just click delete  Drag the animation sequence into here and then hit simulate and it'll start just playing it automatically  That's going to close this one  A bit like that  And if I hit f 11  Yeah  That's great. Well fun. All right, so now I've got my scene that I made earlier  Using composure so I'm going to add him into that now  so here's my scene and  I have got  Let me go into the 3D mode now this might if you're not used to composure and 3D compositing  This might be a little bit  Freaky looking but basically I'm re-projecting this video that I shot of me  Onto a version of this room and these boxes. So if I come out of here  Going to my perspective that's come out of here  So if I zoom out here  This is what I'm doing. So I'm actually  There's our main camera. I've made some boxes and I'm projecting from the camera  The footage that I shot and with the power of unreal  We can add in

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_012.jpg

### 3D Compositing with Composure [12:18]
**Transcript:** characters and it will light the environment and this is called composure and it's fantastic  So there's my scene. There's me in a 3D scene. I'm going to now put that animation onto these robots  So I've got my robot here  And I'm going to go animation plus and I'm going to grab my animation  There we are and I'm going to slip the animation  So that it fits here and I'm also going to move the character. So let's go into his  Transforms  And select him where is he there is and let's bring him forward over here  Like that and let's come and bring him back  And then where's the scene where I've started talking about I go hello like this  So if anyone knows who's used Rikoko  Is it possible to record the audio from your computer mic along with your motion capture at the same time in the same piece of software

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_013.jpg

### Adding the Animated Character to the VFX Scene [13:16]
**Transcript:** So do I have to use like OBS or something like that or whatever to record my audio while I'm doing my oh  Monster blah blah blah with my motion capture  I  Hoping that if I use the facecam stuff it'll allow me to do that as well  But even without the facecam if I'm just using the suit and the Rikoko studio  Is it possible to record the audio and play it back in the same software?  Please let me know in the comments if you know that and if  It's not not possible then dear Rikoko please can you add that for me. Thanks  So there's my animation. I've brought in the one long piece retargeted onto the metahuman skeleton  played around with it inside of unreal and

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_014.jpg

### Question for the Community: Recording Audio in Rokoko Studio? [14:04]
**Transcript:** Slowed it down scale things. I mean, it's phenomenal really that in what's that been an hour and a half two hours  Since when I first put the suit on to recording my animation and putting it into the scene and I'm super happy  So thank you Rikoko for sending me the suit and  I'm a big fan and  Yeah, well fun. I'm gonna go and render this up now and go and edit it and work on the next video  But in the meantime if you interested in this sort of projection techniques and 3d compositing then check out  All of my videos, especially this one  Thanks so much

**Frame:** tutorials\frames\faster-than-ai-and-7-times-the-fun-speed-up-animation-and-get-exactly-what-you-w\frame_015.jpg


---

## Structured Notes

### Core Technique
Rokoko Smartsuit Pro II + Smart Gloves + Coil Pro → Rokoko Studio (calibrate, record, foot lock edit, finalize post-process, export FBX) → UE5 import + retarget → Sequencer + Composure integration.

### Summary
Dean Yurke unboxing and first-use review of the Rokoko Smartsuit Pro II. Setup: ~2 hours from box to working animation. IMU-based (no external cameras), WiFi via PC mobile hotspot. Smart Gloves: drag onto actor in Rokoko Studio; without Coil Pro the 3D spatial sense of hands is limited. Coil Pro (EMF generator): improves hand tracking; sensitive near magnetic cables (jitter in display but NOT in recorded data for offline work). Post-processing in Rokoko Studio: foot lock editor (manually mark planted foot segments), Finalize Post Process. Additional tool: Rokoko Studio Preview for jitter smoothing + loop creation. Export as FBX → import into UE5 → RMB Retarget Animations → choose target mesh → Export Animations → drop retargeted anim into Sequencer. Combined with Composure camera projection for live-action background integration.

### Key Steps
1. **Hardware setup:** install Rokoko Studio + firmware updates; hotspot via PC if no router available; plug in suit + gloves + connect Coil Pro
2. **Create actor in Rokoko Studio:** File → Create New Scene → name; create Actor → drag suit sensor onto actor
3. **Calibrate:** T-pose position → calibrate (6-second delay to walk into pose)
4. **Smart Gloves:** drag each glove onto actor in Rokoko Studio; with Coil Pro: 3D spatial position tracked; without: limited spatial sense
5. **Coil Pro note:** sensitive to magnetic cables/equipment; jitter visible in preview but NOT recorded in offline mode
6. **Record performance:** hit Record; perform; stop recording
7. **Foot lock cleanup:** bottom bar → Foot Lock editor; identify where feet should be planted; manually draw/edit lock segments over recording; remove/add lock sections where feet lift or land
8. **Finalize Post Process:** run to clean up drift and apply foot locking
9. **Optional: Rokoko Studio Preview:** separate software for additional jitter smoothing + loop creation
10. **Export FBX:** export from Rokoko Studio as FBX
11. **Import to UE5:** RMB in Content Browser → Import to Current Folder → find FBX → default import → creates skeletal mesh + animation assets
12. **Retarget animation:** RMB on animation asset → Retarget Animations → source skeletal mesh → choose target skeletal mesh → preview → Export Animations → name/output location → Export
13. **Test in Sequencer:** drag retargeted animation sequence onto character animation track; slip to align timing; adjust character transform in scene
14. **Composure integration:** character added to Composure Shadow/Reflection Layer for shadow casting on projected plate

### UE Systems / Blueprints / Settings
- **FBX Animation Import (UE5)**: RMB → Import to Current Folder; default settings create animation asset with source skeleton; no skeleton reassignment needed if FBX has compatible hierarchy
- **Retarget Animations (UE5)**: RMB on animation → Retarget Animations; pick source and target skeletal mesh; doubles as preview; outputs exportable retargeted animation clips
- **Sequencer Animation Track**: drag animation sequence directly onto character animation slot; can slip timing inline

### Difficulty
Beginner — straightforward workflow, hardware-focused; UE side is simple import + retarget

### UE Version
UE5

### Tags
[mocap, rokoko, animation, retargeting, fbx, sequencer, composure, smartsuit, hand-tracking, beginner]

---

## Related Entries
- budget-mocap-tutorial---quickmagicai-and-metahuman-animator-androidframe-mancer-.md (alternative AI mocap pipeline without hardware)
- easiest-vfx-pipeline-ever-with-composite-mesh-actors-in-unreal-engine-57-composu.md (Composure system used in this tutorial's final shot)
