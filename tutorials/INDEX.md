...
**To add a YouTube tutorial:** say "ingest this: [YouTube URL]"
**To add an Epic documentation section:** say "ingest this: [dev.epicgames.com/documentation URL]"
**To add an Epic community talk:** say "ingest this: [dev.epicgames.com/community URL]"
...
### Unreal Engine Black Eye Cameras: v1.1.7 Switcher + Pilot
- **Source:** YouTube
...
- **Author:** Smart Poly
- **UE Version:** 5.8
- **Tags:** metahuman, mocap, animation, quickstart
...
- **Author:** Unreal Engine
- **UE Version:** UE5 (general, discussion spans older and newer UE5.x releases including UE5.8-era residency defaults)
...
- **Author:** PivotOn
- **UE Version:** Not specified (standard UE5.x features)
...
- **Author:** Unreal Engine
- **UE Version:** UE5 (tail-physics bug reproduced specifically in UE 5.3)
...
- **Author:** Unreal Engine
- **UE Version:** UE 5.8 (Experimental) — Procedural Vegetation Editor (PVE), added 5.7, expanded 5.8
...
- **Summary:** In-depth Lumen console-variable tuning via the Console Variables Editor panel: r.LumenScene.Radiosity.HemisphereProbeResolution (128), r.LumenScene.Radiosity.MaxRayIntensity (4.0), r.LumenScene.Radiosity.Temporal.MaxFramesAccumulated (4096; engine default confirmed as 4 via the panel tooltip Custom/Preset/Startup readout), plus VisualizeProbes / VisualizeProbeRadius as the live debug view; r.Lumen.Reflections.Temporal.MaxFramesAccumulated (4096) and r.Lumen.Reflections.MaxRayIntensity; r.LumenScene.SurfaceCache.CardMaxResolution (4096); r.Lumen.ScreenProbeGather.DownsampleFactor (1 - inverted, LOWER is better, high values black out edge occlusion); r.LumenScene.Radiosity.Temporal.FixedJitterIndex (-1). Fixes GI and reflection flicker WITHOUT touching Final Gather Quality - the Post Process Volume it bypasses runs Lumen reflections with MegaLights enabled. SpecularScale and Reflections.Contrast are demonstrated but never committed (both stay at Constructor defaults in every frame). All twelve variable names were read from captured frames - the narration never says one aloud, and Whisper rendered Radiosity as rigidity throughout.
...
### By UE System
`#niagara` `#sequencer` `#mrq` `#lumen` `#nanite` `#tsr` `#path-tracing`
`#materials` `#shaders` `#pbr` `#post-process` `#lighting` `#volumetrics`
`#blueprints` `#python` `#hlsl` `#cpp` `#pcg` `#chaos`
`#metahuman` `#control-rig` `#animation` `#rigging`
`#audio` `#soundscape`

### By Discipline
`#cinematics` `#camera` `#level-sequence` `#vfx` `#particles` `#gpu-particles`
`#rendering` `#real-time` `#offline-render` `#pipeline` `#automation`
`#modelling` `#geometry` `#environment`
